...
- 축 제목, 범례, panel label 모두 일관되게

**2. 색상 통일**
- 같은 의미를 가진 요소는 같은 색상을 사용
- 예: dnPTV는 항상 빨간색, dnMIS는 주황색, non-carrier는 초록색
- Color picker를 사용해 RGB 값을 정확히 맞춘다

**3. 여백과 정렬**
- 패널 간 여백을 균등하게
- `Align` 패널을 사용해 객체들을 정확히 정렬
- Panel label (A, B, C, D)의 위치를 일관되게

**4. 파일 저장**
- 작업 파일은 `.ai` 형식으로 저장 (편집 가능)
- 최종 제출용은 `.pdf` 형식으로 export
- 해상도는 최소 300 dpi 유지

### 실전 팁

```
# Illustrator에서 자주 사용하는 단축키
Cmd+A (전체 선택)
Cmd+G (그룹화)
Cmd+Shift+O (텍스트를 outline으로 변환)
Shift+Cmd+V (제자리에 붙여넣기)
```

**Panel label 배치 예시:**
- 각 패널의 왼쪽 상단에 "A", "B", "C", "D"를 배치
- 폰트: Arial Bold, 12pt
- 색상: 검은색
- 여백: 패널 테두리에서 5mm 안쪽
...
논문을 작성할 때 (Word 문서 기준) Figure를 삽입하는 방법과 규칙을 알아보자.

### 파일 형식과 저장

**논문 제출용 Figure 파일:**
- **작업 파일**: `.ai` (Adobe Illustrator) — 수정 가능하도록 보관
- **제출 파일**: `.pdf` 또는 `.tiff` — 저널 규정에 따라 선택
- **해상도**: 최소 300 dpi, 가능하면 600 dpi
- **색상 모드**: RGB (온라인용) 또는 CMYK (인쇄용)

### Word 문서에 Figure 삽입

논문을 Word로 작성할 때, Figure는 본문과 별도로 관리하는 것이 일반적이다.

**방법 1: 본문에 직접 삽입**
```
Insert > Picture > 파일 선택
```
- 장점: 바로 위치 확인 가능
- 단점: 파일 크기가 커지고, 저널 제출 시 별도 파일로 분리해야 함

**방법 2: 별도 파일로 관리 (권장)**
- Figure를 별도 폴더에 저장: `Figures/Figure1.pdf`, `Figures/Figure2.pdf`
- Word 본문에는 Figure 위치만 표시:
  ```
  [Figure 1 about here]
  ```
- 논문 끝에 Figure legends 섹션 추가
...
3. **현재형 사용** (Results 섹션은 과거형이지만 legend는 현재형)
...
이제 여러분은 Figure를 그리는 전체 프로세스를 이해했습니다. 처음에는 초안부터 시작하고, 스크립트로 패널을 조합하고, Illustrator로 마무리하는 과정을 반복하세요. Figure가 완성되면, 논문의 절반이 완성된 것입니다.